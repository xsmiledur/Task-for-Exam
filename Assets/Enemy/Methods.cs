...
//
//	/* A,Bの敵の位置が一致しているかどうか。trueなら一致、falseなら一致していない */
//	public bool checkEnemyEqual() {
//		enemyObj = GameObject.Find ("Enemy");
//		GameObject eObjA = GameObject.FindWithTag ("EnemyA");
//		Vector3 Apos = eObjA.transform.position;
//		GameObject eObjB = GameObject.FindWithTag ("EnemyB");
//		Vector3 Bpos = eObjB.transform.position;
//		if (checkVecEqual (Apos, Bpos))
//			return true;
//		else
//
//	}
...
//	/* 現在のプレイヤーの位置に対する、enemyの次の位置を求める */
//	public eInfo[] getNew_eInfos() {
//		// enemyの数だけある
//		eInfo[] new_eInfos = new eInfo[init.eInfos.Length];
//		for (int i = 0; i < init.eInfos.Length; i++) {
//			Vector3 epos = eb.enemyMove (init.eInfos [i], init.playerPos, init.count);
//			new_eInfos [i] = new eInfo (init.eInfos [i].type, epos);
//			//sw.WriteLine ("eNext: " + new_eInfos [i].type + new_eInfos [i].pos);
//		}
//
//		return new_eInfos;
//	}
...
//	/* playerの次の位置に対応する、enemyの次の次の位置を求める */
//	public Vector3[,] getEnemyNextNextPos(eInfo[] new_eInfos) {
//		// playerの次の方向を,停止以外で全て得る
//		Vector3[] move = getMoveDirect(false);
//
//		// 縦はplayerの次の動きの種類数、横はenemyの数とする
//		Vector3[,] eNextPos = new Vector3[0, new_eInfos.Length];
//
//		// playerの次の取り得る位置に応じて、enemyの次の動きを決定
//		for (int i = 0; i < move.Length; i++) {
//			Vector3 playerNextPos = init.playerPos + move [i];
//
//			//sw.WriteLine ("==\nplayerNextPos: " + playerNextPos);
//			if (eb.checkCollider(playerNextPos, true)) {
////			if (eb.checkCollider (playerNextPos, true, "")) { // 壁とぶつからない場合
//
//				Vector3[,] _eNextPos = new Vector3[eNextPos.GetLength(0) + 1, eNextPos.GetLength (1)];
//				for (int k = 0; k < eNextPos.GetLength (0); k++) {
//					for (int j = 0; j < eNextPos.GetLength (1); j++) {
//						_eNextPos [k, j] = eNextPos [k, j];
//					}
//
//				}
//				for (int j = 0; j < eNextPos.GetLength (1); j++) {
//					//_eNextPos [eNextPos.GetLength(0), j] = _eNextVec [j];
//					_eNextPos [eNextPos.GetLength(0), j] = eb.enemyMove (new_eInfos [j], playerNextPos, init.count + 1);
//				}
//
//				eNextPos = _eNextPos;
//			}
...
//	/* 「次の次の」enemy間の相対位置の距離が小さくなるようなplayerの動きを決定する */
//	public Vector3 get_pPos_eWNextDistLeast() {
//		// まず現在のplayerの位置に対するenemyの動く先を求める
//		eInfo[] new_eInfos = getNew_eInfos();
//
//		// playerの次の動く先(vector)を求める
//		Vector3[] move = getMoveDirectWithEnemy(false, true);
//
//		// 次のプレイヤーの動く先それぞれについて、enemyの次の次の位置を求める
//		Vector3[,] eWNextPos = getEnemyNextNextPos(new_eInfos);
//
//
//		Vector3 rltPos;
//		float dist, leastDist = 1000;
//		int leastNo = -1;
//		for (int i = 0; i < eWNextPos.GetLength (0); i++) {
//			rltPos = eWNextPos [i, 0] - eWNextPos [i, 1];
//			dist = Mathf.Abs (rltPos.x) + Mathf.Abs (rltPos.y);
//			if (dist < leastDist) {
//				leastDist = dist;
//				leastNo = i;
//			}
//
//		}
//		//sw.WriteLine ("leastNo: " + leastNo.ToString ());
//
//
//		if (leastNo >= 0) {
//			return init.playerPos + move [leastNo];
//		} else {
//			return init.playerPos;
//		}
//	}

	/* 次のenemyとplayerとの距離が小さくなるようなplayerの動きを決定する */
//	public Vector3 get_pPos_NextNearest() {
//		// Aの次の動きを知る
//		GameObject eObjA = GameObject.FindWithTag ("EnemyA");
//		string Aname = eObjA.name;
//		int index = Int32.Parse(Aname.Substring (5));
//		Vector3 eWNextPos = eb.enemyMove (init.eInfos [index], init.playerPos, init.count);
//
////		Vector3 rltPos;
//		float bfsDist, leastDist = 1000;
//		int leastNo = -1;
//		Vector3[] move = getMoveDirectWithEnemy(false, false); //ここ変更する
//
//		for (int i = 0; i < move.Length; i++) {
//			// 訪問済みの位置にフラグを立てるarrayを初期化
//			bool[,] posHist = createVisitedArray (init.wallEdgePos [3], init.wallEdgePos [0]);
...
//
////			// dynamicaEnemyのキューを作る
////			eInfo[] dnmeCue = bfs_dnmeCuePrepare (false, init.eInfos [0]);
////
////			// countのキューを作る
////			int[] cntcue = bfs_cntcuePrepare (false);
////
//			bfsPos = bfs (bfsPos, cue, eWNextPos, posHist, false, new eInfo[0], new int[0]);
//			bfsDist = bfsPos [0].bfsDist;
//
//			// 距離が3以上で最小なら採用
//			if (bfsDist >= 3 && bfsDist < leastDist) {
//				leastDist = bfsDist;
//				leastNo = i;
//			}
//		}
//
//		if (leastDist == 3 || leastDist == 4) {
//			eb.enemyCopeEnd = true;
//		}
//
//		if (leastNo >= 0) {
//			return init.playerPos + move [leastNo];
